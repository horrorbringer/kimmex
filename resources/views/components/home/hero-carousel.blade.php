            ...
            <template x-for="(slide, index) in slides" :key="'content-'+index">
                <div x-show="current === index"
                    ...
                    <div class="flex items-center gap-4 mb-6">
                        ...
                        <span class="text-titan-red font-bold tracking-[0.2em] uppercase text-sm" x-text="slide.subtitle"></span>
                    </div>

                    <h1 class="font-heading font-black mb-6 text-5xl md:text-7xl leading-[1.1] tracking-tight" x-text="slide.title"></h1>

                    <p class="text-white/80 max-w-lg mb-10 font-light text-lg md:text-xl leading-relaxed" x-text="slide.desc"></p>

                    <div class="flex flex-wrap gap-4">
                        <a :href="slide.link" class="group bg-accent-orange text-white px-8 py-4 font-bold text-sm tracking-widest uppercase hover:bg-white hover:text-titan-navy transition-all duration-300 flex items-center gap-3 rounded-none">
                            <span>{{ __('View Project') }}</span>
                            <x-lucide-arrow-right class="group-hover:translate-x-1 transition-transform w-[18px] h-[18px]" />
                        </a>
                        <a href="/contact" class="group border-2 border-white text-white px-8 py-4 font-bold text-sm tracking-widest uppercase hover:bg-white hover:text-titan-navy transition-all duration-300 flex items-center gap-3 rounded-none">
    ...
    <div class="absolute bottom-12 left-0 right-0 z-20">
        <div class="max-w-[1400px] mx-auto px-6 flex items-end justify-between">
            ...
            <div class="flex gap-4">
                <template x-for="(slide, index) in slides" :key="'dot-'+index">
                    <button @click="goToSlide(index)" 
                            :class="index === current ? 'w-16 bg-titan-red' : 'w-8 bg-white/30 hover:bg-white/60'"
            ...
            <div class="flex gap-2">
                <button @click="prevSlide" class="w-12 h-12 border border-white/20 rounded-full flex items-center justify-center hover:bg-accent-orange hover:border-accent-orange transition-all duration-300 text-white">
                    <x-lucide-chevron-left class="w-6 h-6" />
                </button>
                <button @click="nextSlide" class="w-12 h-12 border border-white/20 rounded-full flex items-center justify-center hover:bg-accent-orange hover:border-accent-orange transition-all duration-300 text-white">
    ...
    <div class="hidden lg:block absolute bottom-12 right-[20%] z-10">
        <div class="flex gap-12">
            <div>
                ...
                <div class="text-[10px] text-accent-orange uppercase tracking-widest font-bold">{{ __('Years Exp') }}</div>
            </div>
            <div>
