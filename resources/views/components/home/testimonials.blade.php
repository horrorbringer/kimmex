...
<section class="py-24 bg-white">
    <div class="max-w-[1400px] mx-auto px-6">
        <div x-data="{ shown: false }" x-intersect.once="shown = true" :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'" class="text-center max-w-2xl mx-auto mb-16 transition-all duration-1000">
            <span class="text-accent-orange font-bold uppercase tracking-widest text-sm mb-4 block">{{ __('Testimonials') }}</span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach($testimonials as $index => $t)
                <div x-data="{ shown: false }" x-intersect.once="shown = true" style="transition-delay: {{ $index * 100 }}ms" :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'" class="transition-all duration-1000">
                    <div class="bg-gray-50 p-8 rounded-2xl relative h-full flex flex-col group hover:-translate-y-2 hover:shadow-xl transition-all duration-500">
                        <x-lucide-quote class="text-accent-orange/20 absolute top-6 right-6 w-12 h-12 group-hover:text-accent-orange/40 transition-colors" />
                        <div class="flex gap-1 mb-4">
                            @for($i = 0; $i < $t['rating']; $i++)
                                <x-lucide-star class="text-accent-orange fill-accent-orange w-4 h-4" />
                            @endfor
                        </div>
                        <p class="text-titan-navy/70 mb-6 relative z-10 leading-relaxed flex-grow">&ldquo;{{ $t['quote'] }}&rdquo;</p>
                        <div class="flex items-center gap-4">
                            <div class="w-12 h-12 bg-titan-navy rounded-full flex items-center justify-center text-white font-bold shrink-0">
                                {{ substr($t['author'], 0, 1) }}
                            </div>
                            <div>
