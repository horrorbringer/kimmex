<x-layouts.app title="Projects" description="View Kimmex's portfolio of successful construction and engineering projects.">
...
<div x-data="{
    ...
    <section class="relative z-10 flex items-center justify-center overflow-hidden bg-titan-navy min-h-[85vh] projects-hero-container">
        <!-- Cinematic Background -->
        <div class="absolute inset-0 scale-105">
            <img src="/images/projects/Thumbnail-5.jpg" alt="Kimmex Built Legacy" class="w-full h-full object-cover opacity-30 blur-[1px]" />
            <div class="absolute inset-0 bg-gradient-to-t from-titan-navy via-titan-navy/90 to-transparent"></div>
        </div>

        <!-- Decorative Elements -->
        <div class="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>
        <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[1200px] h-[800px] bg-titan-red/5 rounded-full blur-[150px] pointer-events-none"></div>

        <div class="relative z-20 text-center px-6 max-w-7xl pt-24" x-data="{ shown: false }" x-init="setTimeout(() => shown = true, 100)">
            <div :class="shown ? 'opacity-100' : 'opacity-0'" class="transition-all duration-1000 delay-100 inline-flex items-center gap-3 px-5 py-2.5 bg-white/5 backdrop-blur-xl rounded-lg border border-white/10 text-white text-[10px] font-black uppercase tracking-[0.3em] mb-12 shadow-2xl">
            ...
            <h1 :class="shown ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'" class="transition-all duration-1000 delay-300 text-5xl md:text-8xl font-black text-white mb-8 leading-[0.9] tracking-tighter uppercase">
                {{ __('BUILT') }} <span class="text-titan-red">{{ __('LEGACY') }}</span>
            </h1>

            <p :class="shown ? 'opacity-100' : 'opacity-0' " class="transition-all duration-1000 delay-500 text-[10px] md:text-xs text-white/40 max-w-2xl mx-auto leading-loose font-bold uppercase tracking-[0.4em] mb-16">
                {{ __('Architecting the future through engineering precision.') }}
            </p>
        </div>

        <!-- Scroll Indicator -->
        <div class="absolute bottom-10 left-1/2 -translate-x-1/2 flex flex-col items-center gap-4 cursor-pointer group z-20" onclick="document.getElementById('portfolio-grid').scrollIntoView({ behavior: 'smooth' })">
            <span class="text-[9px] uppercase tracking-[0.4em] font-black text-white/30 group-hover:text-white transition-colors">{{ __('Scroll') }}</span>
    ...
    <section id="portfolio-grid" class="py-24 px-6 bg-white relative">
        <div class="max-w-[1700px] mx-auto">
            ...
            <div class="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 mb-16 border-b border-gray-200 pb-6">
                
                <!-- Category Tabs -->
                <nav class="flex flex-wrap items-center gap-2 lg:gap-8 w-full xl:w-auto overflow-x-auto no-scrollbar">
                ...
                <div class="flex flex-wrap items-center gap-4 w-full xl:w-auto">
                    ...
                    <div class="flex items-center gap-3 bg-gray-50 border border-gray-100 rounded-xl p-1 shrink-0">
                        ...
                        <div class="relative min-w-[130px]">
                            <select x-model="filterLoc" class="appearance-none w-full bg-transparent pl-4 pr-10 py-2.5 text-[10px] font-black uppercase tracking-widest text-titan-navy transition-all cursor-pointer focus:outline-none focus:ring-0 border-0">
                                <template x-for="loc in locations" :key="loc">
                                    <option :value="loc" x-text="loc === 'All' ? 'All Locations' : loc"></option>
                                </template>
                            </select>
                            <x-lucide-chevron-down class="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-titan-navy/40 pointer-events-none" />
                        </div>
                        ...
                        <!-- Status Badge Selector -->
                         <div class="relative min-w-[130px]">
                            <select x-model="filterStatus" class="appearance-none w-full bg-transparent pl-4 pr-10 py-2.5 text-[10px] font-black uppercase tracking-widest text-titan-navy transition-all cursor-pointer focus:outline-none focus:ring-0 border-0">
                                <template x-for="stat in statusOptions" :key="stat">
                                    <option :value="stat === 'All' ? 'All Status' : stat" x-text="stat === 'All' ? 'Project Status' : stat"></option>
                                </template>
                            </select>
                            <x-lucide-chevron-down class="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-titan-navy/40 pointer-events-none" />
                        </div>
                    ...
                    <div class="relative shrink-0 w-full sm:w-auto">
                        <x-lucide-search class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-titan-navy/30" />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <template x-for="project in filteredProjects" :key="project.id">
                    <div class="animate-fade-in-up h-full">
                        <a :href="'/projects/' + project.id" class="group block relative bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 flex flex-col hover:-translate-y-2 hover:shadow-2xl transition-all duration-500 h-full">
                            ...
                            <div class="relative w-full aspect-[16/10] overflow-hidden bg-gray-100 shrink-0">
                                ...
                                <div class="absolute inset-0 bg-titan-navy/0 group-hover:bg-titan-navy/10 transition-colors duration-500"></div>
                                
                                <!-- Status Overlay -->
                                <div class="absolute top-4 left-4 z-20">
                                    <span class="px-3 py-1.5 backdrop-blur-md rounded border border-white/20 text-white text-[9px] font-black tracking-widest uppercase shadow-lg"
                            ...
                            <div class="p-8 flex flex-col flex-1">
                                <!-- Red Accent Bar -->
                                <div class="w-8 h-1 bg-titan-red mb-5 group-hover:w-12 transition-all duration-300"></div>
                                
                                <h3 class="text-xl font-black text-titan-navy leading-tight mb-2 group-hover:text-titan-red transition-colors uppercase tracking-tight" x-text="project.title"></h3>
                                <p class="text-titan-navy/40 text-[10px] font-black uppercase tracking-widest mb-4" x-text="project.type"></p>
                                
                                <p class="text-gray-500 text-sm leading-relaxed mb-6 font-medium line-clamp-2" x-text="project.summary"></p>
                                ...
                                <div class="mt-auto pt-5 border-t border-gray-100 flex items-center justify-between">
                                    ...
                                    <div class="text-[10px] font-black uppercase tracking-widest flex items-center gap-2 text-titan-navy group-hover:text-titan-red transition-colors">
                                        {{ __('View Details') }}
                                        <x-lucide-arrow-right class="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                                    </div>
                                </div>
                            </div>
                        </a>
            ...
            <!-- Empty State -->
            <div x-show="filteredProjects.length === 0" style="display: none;" class="text-center py-40 bg-gray-50 rounded-[4rem] border border-dashed border-gray-100">
                <x-lucide-building class="w-12 h-12 text-titan-navy/10 mx-auto mb-8" />
                <h3 class="text-2xl font-black text-titan-navy mb-4 uppercase tracking-tighter">{{ __('No Built Legacy Found') }}</h3>
                <p class="text-titan-navy/40 text-sm max-w-sm mx-auto leading-relaxed">{{ __('Refine your search parameters to explore other successful Kimmex deliveries.') }}</p>
            </div>
        </div>
    </section>
        </div>
